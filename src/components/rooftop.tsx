...
const CHECKLISTS: Record<string, string[]> = {
	...
	close: [
		...
		'Empty Ice',
		'Empty and Flip bins',
		'Returned or Brought back all bar equipment',
		'Lock the doors',
		'Lock the bar',
		'Cash off',
...
export const Rooftop: React.FC = () => {
	useEffect(() => {
		...
		document.getElementsByTagName('body')[0].setAttribute('style', 'background: #FFF; color: #000;');
	}, []);
			...
			<DialogContent>
				...
				<div>
					{CHECKLISTS[checklist].map((instruction) => {
						return <FormControlLabel key={instruction} control={<Checkbox />} label={instruction} />;
					})}
				</div>
			</DialogContent>
		</Dialog>
